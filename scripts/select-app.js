...
function writeAndroidManifest(appKey) {
  ...
  /* THE REPLACEMENT USED TO MISS ITS OWN HEADER.
     The generated comment was written BEFORE the first <uses-permission>, and
     the span replaced on the next build started AT the first
     <uses-permission> — so every run left its comment behind and added
     another. Sixteen of them had piled up by the time an audit noticed, each
     naming a different build, so the file's own header contradicted itself and
     three of its predecessors about which app it belonged to.

     Explicit markers now bound the block, so a rewrite replaces exactly what
     the previous rewrite wrote. The uses-permission span stays as the
     fallback for the first run on a freshly generated project, which has no
     markers yet. */
  const START = "<!-- NOVAGO:PERMISSIONS:START -->";
  const END = "<!-- NOVAGO:PERMISSIONS:END -->";
  ...
  const mStart = xml.indexOf(START);
  const mEnd = xml.indexOf(END);
  if (mStart !== -1 && mEnd !== -1) {
    xml = xml.slice(0, mStart) + block.trimStart() + xml.slice(mEnd + END.length);
  } else {
    // First run on a fresh project: sweep any pre-marker generated comments
    // out too, or this run inherits the pile it exists to prevent.
    xml = xml.replace(
      /[ \t]*<!-- GENERATED by scripts\/select-app\.js[\s\S]*?-->\n?/g,
      "",
    );
